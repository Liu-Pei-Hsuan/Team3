...
# 分成訓練集和驗證集
# =============================================================================
def Train_Test_Data(df, feature, size = 0.8): 
    '''
...
# 各種 plot
# =============================================================================
def timeseries_plot(df, feature, train, valid):
    # 原始時間序列圖
    plt.figure(figsize=(20, 10))
    df_plot = df[feature]
    df_plot.plot()
...
# 配適模型
# =============================================================================
def arima_model(feature, bestModel):
    model = ARIMA(data[[feature]], order = bestModel)
    global model_fit
    model_fit = model.fit()
...
def pred_plot(train, test, pred, start, end):
    plt.rcParams.update({'figure.figsize':(10, 5)})
